...
@dp.table(
    comment="BLS PR current year-to-date data"
)
...
def brz_pr_data_0_current():
    ...
    df = spark.read.options(**bls_options).schema(schema).csv('/Volumes/datasets/default/bls/pr/pr.data.0.Current')
